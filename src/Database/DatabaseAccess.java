...
public class DatabaseAccess 
{
	...
	public void addTestCase(String input, String output, int testId) throws SQLException {
		
		Connection con = null;
		PreparedStatement pt = null;
		try {
			con = getConnection();
			pt = con.prepareStatement("Insert into TestCase (input, output, testId) values (?,?,?)");
	...
	public Set<Test> getTests() {
		...
		try {
			...
			while (rs.next()) 
			{
				...
				tests.add(new Test(testId, testTitle, testDesc, testDiff, testcases));
			}
		} catch (Exception e) {
			System.out.println(e);
		} finally {
	...
	private Set<TestCase> getTestCases(int testId) {
		...
		try {
			...
			con = getConnection();
			pt = con.prepareStatement("Select * from TestCase where testId = ?");
			pt.setInt(1, testId);
			rs = pt.executeQuery();
			String p[] = new String[5];
			while (rs.next()) 
